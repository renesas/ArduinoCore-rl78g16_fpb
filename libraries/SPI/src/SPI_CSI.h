...
public:
  ...
  inline uint16_t getMasterTxCount   (void) const { return master_tx_count    == nullptr ? 0U      : *master_tx_count   ; }
  inline uint16_t getMasterStatusFlag(void) const { return master_status_flag == nullptr ? 0x0000U : *master_status_flag; };
...
private:
  ...
  const unsigned int STATUS_WAIT_TIMEOUT   = 1000;
  // const unsigned long STATUS_WAIT_INTERVAL  = 10;
  const unsigned int STATUS_WAIT_INTERVAL  = 1;
